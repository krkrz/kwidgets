...
- **id** (string: "")
  - ID
	 ...
	 IDを設定することで特定のスタイルと対応させられます。 ( [親のスタイルのIDプロパティで指定する場合](Style.md#親のスタイルのIDプロパティで指定する場合), [グローバルなスタイルレポジトリのIDスタイルで指定する場合](Style.md#グローバルなスタイルレポジトリのIDスタイルで指定する場合) )

- **style** (dictionary: %[])
  - スタイル

	 ウィジェットの見た目を定義する[スタイル](Style.md)を指定します。
